 ...
 <section class="padding-top-50 padding-bottom-50">
    <div class="container">
      ...
      <div class="heading text-center">
        ...
        <span>Có tất cả {{count($show_product_by_cate)}} sản phẩm</span> </div>

      <!-- Popular Item Slide -->
      <div class="papular-block block-slide">
        <!-- Item -->
        @foreach ($show_product_by_cate as $key => $product)
        <div class="item">
            <!-- Item img -->
            ...
            <div class="item-name" data-toggle="tooltip" data-placement="top" title="Chi tiết {{$product->product_name}}"><a href="{{URL::to('chi-tiet-san-pham/'.$product->product_slug)}}">{{$product->product_name}}</a>
              ...
              <input type="hidden" name="productid_hidden" value="{{$product->product_id}}"><br>
                    <form action="">
                        ...
                        @endphp
                        <input type="hidden" value="{{$product->product_sale_after}}" class="cart_product_sale_after_{{$product->product_id}}">
